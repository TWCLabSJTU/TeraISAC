function [recoveredData, X_eq] = DFTSOTFSCommReceiver(rxSignal, waveformPar, SNRdB, pathNumber)
...
X_eq = X_eq - pilot;
X_eq = ifft(X_eq.').' * sqrt(N);
X_de = demodulation(X_eq, modSize, modType);
nEq = 5;
for n = 1:nEq
    data_star = modulation(X_de, modSize, modType);
    data_star_dd = fft(data_star.').' / sqrt(N);
