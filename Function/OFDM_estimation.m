function [alpha, delay, Doppler] = OFDM_estimation(Xtf, Ytf, deltaf, Ts, P, K)
[M, N] = size(Xtf);
alpha = zeros(1, P);
delay = zeros(1, P);
Doppler = zeros(1, P);
...
ytf_ni = ytf;
for p = 1:P
    Ytf = reshape(ytf_ni, [M N]);
    % phase I
    delayList = 0:1:(M-1);
    DopplerList = 0:1:(N-1);
    profile = fft(ifft(conj(Xtf) .* Ytf).').';
    ...
    for k = 1:K
        ...
        f11 = abs(sum(sum(conj(Ytf_11) .* Ytf)))^2;
        f12 = abs(sum(sum(conj(Ytf_12) .* Ytf)))^2;
        f21 = abs(sum(sum(conj(Ytf_21) .* Ytf)))^2;
        f22 = abs(sum(sum(conj(Ytf_22) .* Ytf)))^2;
